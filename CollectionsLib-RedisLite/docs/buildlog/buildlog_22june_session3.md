...
* Learned that `malloc()` allocates a block of memory at runtime and returns a pointer to the beginning of the allocated memory.
* Explored the syntax and parameters of `malloc()`.
* Compared stack memory and heap memory to understand where dynamically allocated memory is stored.
* Read about the importance of releasing dynamically allocated memory to avoid memory leaks.
* Took notes on how `malloc()` can be used as the foundation for implementing custom data structures such as a dynamic array.
